...
export default function CalendarDayCard(props: Readonly<CalendarDayCardProps>) {
    return (
        <button className={props.isToday(props.calendarDay.day) ? "day_selected" : "day"}>
            ...
            <CircularProgress variant={"determinate"} value={100} style={{position: "absolute", color:"lightgray"}}/>
            <CircularProgress variant={"determinate"} value={props.percentage} style={{position: "absolute"}}/>
        </button>
    );
}
